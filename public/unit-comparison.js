$(function(){
    ...
    $.getJSON('data.json', function(data) {
        ...
        function applyUpgradeEffects(unitData, locked, currentUpgradeData, selectedCiv, elm){
            function applyEffect(key, data, upgradeData){
                ...
                //Special case for chemistry since it applies to either mAtk or pAtk
                    ...
                    let primaryAtk = (unitData.mAtk || 0) > (unitData.pAtk || 0) ? "mAtk" : "pAtk";
            ...
            for(let i in currentUpgradeData.effects){
                ...
                if(i === "unitSpec"){
                    ...
                    let effects = currentUpgradeData.effects[i][unitData.unitClass] || currentUpgradeData.effects[i][unitData.name];
                    for(j in effects){
        ...
        function applyUpgrade(force){
            ...
            if(upgradeData){
                let currentUpgradeData = locked === 1 ? $.extend({}, upgradeData) : unitData.activeUpgrades.find(function(elm){return elm.name === upgradeData.name;});
                if(currentUpgradeData){
                    ...
                    if(upgradeData.classChange){
                        ...
                        if(unitData.reloadUpgrade){
                            ...
                            for(let i = 0 ; i < unitData.reloadUpgrade.length; i++){
                                for(let j = 0; j < unitData.activeUpgrades.length; j++){
                                    if(unitData.reloadUpgrade[i] === unitData.activeUpgrades[j].name){
                        ...
                        if(locked > 0){
                            ...
                            let portraitURL = $(unlocked[unlocked.length - 1]).attr("upgrade") || unitData.name;
                            portraitCont[0].src = "img/" + portraitURL  + ".png";
                            unitData.unitClass = portraitURL;
                        }
                    }
                    let selectedCiv = elm.closest(".combatant-props").children(".combatant-upgrades").children(".civilization").children("select").children("option:selected").val();
                    //Add/Remove each of the effects
                    applyUpgradeEffects(unitData, locked, currentUpgradeData, selectedCiv, elm);
        ...
        function generateResult(attacker, defender){
            ...
            let elm = $('<div class="combat-result">\n\
                    ...
                    <div class="combat-results">\n\
                        ...
                        <div class="stat-cont">\n\
                            ...
                            <div class="stat">'+finalDamagePerHit+'</div>\n\
                        </div>\n\
                        <div class="stat-cont">\n\
                            <div class="stat-title">Overkill</div>\n\
                            <div class="stat">'+overkill+' (' + minDamageToKill + ')</div>\n\
                        </div>\n\
                        <div class="stat-cont">\n\
            ...
            let switchButton = $("<div class='switch-button'><div class='button'>Swap Combatants</div></div>");
        ...
        function getUpgrades(data, civ){
            let upgradeData = [];
            let elm = $('<div class="combatant-upgrades"><div class="civilization"></div><div class="upgrades"></div></div>');
            ...
            civSelect.on("change", function(){
                ...
                let combatant = createCombatant(combatantDiv.children(".combatant-title").children("h3").text(), data, $(this).children("option:selected").val());
                
                let combatIdx = $("#combatants").children(".combat-cont").index(combatantDiv.closest(".combat-cont"));
                
                let combatantIdx = combatantDiv.parent().children(".combatant").index(combatantDiv);
                combats[combatIdx][combatantIdx] = combatant.data;
        ...
        function createCombatant(title, data, civ){
            ...
            combatantData.bonus = combatantData.bonus ? combatantData.bonus.map(a => Object.assign({}, a)) : [];
            ...
            let primaryAtk = (combatantData.mAtk || 0) > (combatantData.pAtk || 0) ? "mAtk" : "pAtk";
            let unitStats = 
                    '<div>\n\
        ...
        $("#add-combat").click(() => {
            ...
            $(attacker.element).children(".combatant-props").children(".combatant-stats").children("div:eq(0)").children(".icon:eq("+(attacker.data.ageReq-1)+")").click();
            $(defender.element).children(".combatant-props").children(".combatant-stats").children("div:eq(0)").children(".icon:eq("+(defender.data.ageReq-1)+")").click();
        });
        $("#add-combat").click();
    });
